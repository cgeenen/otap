...
docker stop mysonar myjenkins artifactory tomcat 2>/dev/null
...
docker run -d --rm -p 8888:8080 --name tomcat mytomcat:latest
...
docker run -d -p 8080 -v `pwd`/downloads:/var/jenkins_home/downloads \
    ...
    -v `pwd`/jmeter:/var/jenkins_home/jmeter/ \
    -v `pwd`/workspace:/var/jenkins_home/jobs/Deploy/workspace/ \
    -v `pwd`/m2deps:/var/jenkins_home/.m2/repository/ --rm --name myjenkins \
    -e SONARQUBE_HOST=http://10.0.2.15:${sonar_port} \
    -e ARTIFACTORY_URL=http://10.0.2.15:${artifactory_port}/artifactory/example-repo-local \
    myjenkins:latest

echo "Sonarqube is running at http://10.0.2.15:${sonar_port}"
echo "Artifactory is running at http://10.0.2.15:${artifactory_port}"
echo "Jenkins is running at http://10.0.2.15:$(getContainerPort myjenkins)"
